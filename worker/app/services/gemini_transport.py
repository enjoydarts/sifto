...
async def get_or_create_cached_content_async(model: str, api_key: str, cache_key: str, system_instruction: str, *, normalize_model_name, logger) -> str | None:
    now = time.time()
    skip_until = _GEMINI_CONTEXT_CACHE_SKIP.get(cache_key)
    if skip_until and skip_until > now:
        return None
...
async def generate_content_async(
    ...
    *,
    ...
    if system_instruction:
        ...
        if context_cache_key:
            try:
                cached_name = await get_or_create_cached_content_async(
                    model_name,
                    api_key,
                    context_cache_key,
                    system_instruction,
    ...
    for i in range(attempts):
        try:
            ...
            last_error = e
            if i < attempts - 1:
                await asyncio.sleep(base_sleep_sec * (2**i))
                continue
            raise RuntimeError(f"gemini generateContent request failed: {e}") from e
        if resp.status_code < 400:
            break
