...
class FeedTaskCommonTests(unittest.TestCase):
    ...
    def test_build_audio_briefing_script_task_uses_full_persona_and_fixed_article_order(self):
        ...
        self.assertIn("共通点や流れを2〜4点ほど具体化してよい", prompt)
        ...
        self.assertIn("headline では、その記事の短い要約をしてよい", prompt)
        self.assertIn("headline は短い導入見出しとして使い", prompt)
        self.assertIn("commentary では記事の説明、背景整理、論点整理、一般論、今後の含意の解説は禁止", prompt)
        self.assertIn("headline で置いた要約をなぞり直さず、反応と理由に進む", prompt)
