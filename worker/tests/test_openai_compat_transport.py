...
class RunChatJsonTests(unittest.TestCase):
    ...
    @patch("app.services.openai_compat_transport.httpx.Client", _FakeClient)
    def test_zai_glm_5_3_family_requests_use_required_thinking_with_low_effort(self):
        for model in ("glm-5.3", "glm-5.3-flash", "glm-5.3-flashx"):
            with self.subTest(model=model):
                run_chat_json(
                    ...
                    supports_strict_schema=lambda value: False,
                    timeout_sec=5,
                    attempts=1,
                    base_sleep_sec=0,
                    provider_name="zai",
    ...
    @patch("app.services.openai_compat_transport.httpx.Client", _FakeClient)
    def test_deepseek_flash_requests_disable_thinking(self):
        run_chat_json(
            "Return JSON",
            "deepseek-flash",
            "test-key",
