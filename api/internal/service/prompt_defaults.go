...
func defaultPromptVariablesSchema(promptKey string) json.RawMessage {
  ...
  "existing_context": {"type": "string"}
	...
	case "fish.summary_preprocess", "gemini.summary_preprocess", "elevenlabs.summary_preprocess", "xai.summary_preprocess":
		return mustRawJSON(`{
  "text": {"type": "string"}
}`)
...
func defaultPromptNotes(promptKey string) string {
	...
	case "xai.summary_preprocess":
		return "xAI TTS の Summary Audio 向け前処理 prompt です。inline / wrapping tag を使います。"
	case "xai.audio_briefing_single_preprocess":
		return "xAI TTS の Audio Briefing Single 向け前処理 prompt です。"
	case "xai.audio_briefing_duo_preprocess":
		return "xAI TTS の Audio Briefing Duo 向け前処理 prompt です。"
	default:
		return ""
	}
}
