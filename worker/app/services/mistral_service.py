...
_MISTRAL_PRICING_SOURCE_VERSION = "mistral_static_2026_03"
...
def _pricing_for_model(model: str, purpose: str) -> dict:
    ...
    source = str(base.get("pricing_source") or _MISTRAL_PRICING_SOURCE_VERSION)
    prefix = f"MISTRAL_{purpose.upper()}_"
    override_map = {
...
def _llm_meta(model: str, purpose: str, usage: dict) -> dict:
    ...
    return with_execution_failures({
        ...
        "pricing_source": pricing.get("pricing_source", _MISTRAL_PRICING_SOURCE_VERSION),
...
def _chat_json(
    ...
    *,
    ...
    req_timeout = timeout_sec if timeout_sec and timeout_sec > 0 else _env_timeout_seconds("MISTRAL_TIMEOUT_SEC", 300.0)
    attempts = max(1, int(os.getenv("MISTRAL_RETRY_ATTEMPTS", "3") or "3"))
    base_sleep_sec = _env_timeout_seconds("MISTRAL_RETRY_BASE_SEC", 0.5)
    return run_chat_json(
        prompt,
        model,
        api_key,
        url=os.getenv("MISTRAL_API_BASE_URL", "https://api.mistral.ai/v1/chat/completions"),
...
def extract_facts(title: str | None, content: str, model: str, api_key: str) -> dict:
    ...
    if not facts:
        raise RuntimeError(f"mistral extract_facts parse failed: response_snippet={text[:500]}")
...
def summarize(title: str | None, facts: list[str], source_text_chars: int | None = None, model: str = "mistral-small-2506", api_key: str = "") -> dict:
    ...
    return finalize_summary_result(
        ...
        error_prefix="mistral summarize parse failed",
        response_text=text,
    )
...
def translate_title(title: str, model: str = "ministral-8b-2512", api_key: str = "") -> dict:
    src = (title or "").strip()
    if not src:
...
def compose_digest(digest_date: str, items: list[dict], model: str, api_key: str) -> dict:
    ...
    text, usage = _chat_json(
        ...
        response_schema=task["schema"],
        schema_name="digest",
        timeout_sec=_env_timeout_seconds("MISTRAL_COMPOSE_DIGEST_TIMEOUT_SEC", 300.0),
    )
    subject, body = parse_digest_result(text, error_prefix="mistral compose_digest parse failed")
...
def ask_question(query: str, candidates: list[dict], model: str, api_key: str) -> dict:
    ...
    result = parse_ask_result(text, candidates, error_prefix="mistral ask missing answer")
...
def compose_digest_cluster_draft(cluster_label: str, item_count: int, topics: list[str], source_lines: list[str], model: str, api_key: str) -> dict:
    ...
    try:
        ...
        _log.warning("mistral compose_digest_cluster_draft primary attempt failed: %s", exc)
        try:
            ...
            _log.warning("mistral compose_digest_cluster_draft fallback failed: %s", retry_exc)
...
def suggest_feed_seed_sites(existing_sources: list[dict], preferred_topics: list[str], positive_examples: list[dict] | None, negative_examples: list[dict] | None, model: str, api_key: str) -> dict:
    ...
    if len(out) == 0:
        try:
            ...
            _log.warning("mistral suggest_feed_seed_sites rescue failed: %s", exc)
