...
type appDeps struct {
	...
	resend         *service.ResendClient
	oneSignal      *service.OneSignalClient
	secretCipher   *service.SecretCipher
	clerkVerifier  *service.ClerkTokenVerifier
...
func initDeps(ctx context.Context) (*appDeps, service.JSONCache, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("json cache: %w", err)
	}
	search, err := service.NewMeilisearchServiceFromEnv()
	if err != nil {
		return nil, nil, fmt.Errorf("meilisearch: %w", err)
	}
	eventPublisher, err := service.NewEventPublisher()
	if err != nil {
		return nil, nil, fmt.Errorf("event publisher: %w", err)
	}
	...
	keyProvider := service.NewUserKeyProvider(userSettingsRepo, secretCipher)
...
func buildItemsModule(d *appDeps) appModule {
	db := d.db
	itemRepo := d.itemRepo
	...
	userSettingsRepo := d.userSettingsRepo
	llmUsageRepo := d.llmUsageRepo

	itemH := handler.NewItemHandler(itemRepo, sourceRepo, readingGoalRepo, streakRepo, snapshotRepo, prefProfileRepo, reviewQueueRepo, userSettingsRepo, llmUsageRepo, d.eventPublisher, d.secretCipher, d.worker, d.cache, d.search, d.keyProvider)
	notesH := handler.NewItemNotesHandler(itemRepo, reviewQueueRepo, d.eventPublisher)
...
func buildSourcesModule(d *appDeps) appModule {
	db := d.db
	sourceRepo := d.sourceRepo
	itemRepo := d.itemRepo
	...
	sourceH := handler.NewSourceHandler(sourceRepo, itemRepo, sourceOptimizationRepo, userSettingsRepo, llmUsageRepo, d.worker, d.secretCipher, d.eventPublisher, d.cache, d.keyProvider)
...
func buildSettingsModule(d *appDeps) appModule {
	db := d.db
	userRepo := d.userRepo
	userSettingsRepo := d.userSettingsRepo
	...
	obsidianExportSvc := service.NewObsidianExportService(d.itemRepo, repository.NewItemExportRepo(db), obsidianExportRepo, d.githubApp)

	settingsH := handler.NewSettingsHandler(userSettingsRepo, userRepo, audioBriefingRepo, summaryAudioRepo, aivisModelRepo, obsidianExportRepo, notificationPriorityRepo, prefProfileRepo, llmUsageRepo, openRouterModelOverrideRepo, d.secretCipher, d.githubApp, obsidianExportSvc, d.worker, d.cache)
	...
	return appModule{
		registerAPI: func(r chi.Router) {
			r.Route("/settings", func(r chi.Router) {
				...
				r.Get("/ui-font-catalog", settingsH.GetUIFontCatalog)
				r.Patch("/", settingsH.UpdateBudget)
				r.Patch("/ui-fonts", settingsH.UpdateUIFontSettings)
				r.Patch("/audio-briefing", settingsH.UpdateAudioBriefing)
				r.Get("/summary-audio", settingsH.GetSummaryAudioVoiceSettings)
				r.Put("/summary-audio", settingsH.UpdateSummaryAudioVoiceSettings)
				...
				r.Post("/plamo-key", settingsH.SetPLaMoAPIKey)
				r.Delete("/plamo-key", settingsH.DeletePLaMoAPIKey)
...
func buildAudioBriefingModule(d *appDeps) appModule {
	db := d.db
	userRepo := d.userRepo
	userSettingsRepo := d.userSettingsRepo
	audioBriefingRepo := repository.NewAudioBriefingRepo(db)
	audioBriefingPresetRepo := repository.NewAudioBriefingPresetRepo(db)
	summaryAudioRepo := repository.NewSummaryAudioVoiceSettingsRepo(db)
	aivisModelRepo := repository.NewAivisModelRepo(db)
	obsidianExportRepo := repository.NewObsidianExportRepo(db)
	llmUsageRepo := d.llmUsageRepo
	...
	ttsMarkupPreprocessSvc := service.NewTTSMarkupPreprocessService(userSettingsRepo, d.secretCipher, d.worker, llmUsageRepo, d.cache)
	audioBriefingVoiceRunner := service.NewAudioBriefingVoiceRunner(audioBriefingRepo, userRepo, userSettingsRepo, d.secretCipher, d.worker, ttsMarkupPreprocessSvc)
	...
	audioBriefingOrchestrator := service.NewAudioBriefingOrchestrator(audioBriefingRepo, userSettingsRepo, llmUsageRepo, promptResolver, d.secretCipher, d.worker, d.cache, audioBriefingVoiceRunner, audioBriefingConcatStarter)
	audioBriefingDeleteSvc := service.NewAudioBriefingDeleteService(audioBriefingRepo, d.worker)
	audioBriefingPublishedNotifier := service.NewAudioBriefingPublishedNotifier(userRepo, repository.NewNotificationPriorityRepo(db), pushNotificationLogRepo, d.oneSignal, nil)
	podcastPublicationSvc := service.NewPodcastPublicationService(audioBriefingRepo, d.worker)

	audioBriefingsH := handler.NewAudioBriefingsHandler(audioBriefingRepo, audioBriefingOrchestrator, audioBriefingVoiceRunner, audioBriefingConcatStarter, audioBriefingDeleteSvc, d.eventPublisher, d.worker)
	summaryAudioPlayerSvc := service.NewSummaryAudioPlayerService(itemRepo, summaryAudioRepo, userRepo, userSettingsRepo, d.secretCipher, d.worker, ttsMarkupPreprocessSvc)
	...
	aiNavigatorBriefSvc := service.NewAINavigatorBriefService(aiNavigatorBriefRepo, itemRepo, userSettingsRepo, userRepo, pushNotificationLogRepo, llmUsageRepo, d.worker, d.secretCipher, d.oneSignal, d.eventPublisher, d.cache, nil)
	...
	podcastFeedSvc := service.NewPodcastFeedService(userSettingsRepo, audioBriefingRepo, d.worker)
	podcastsH := handler.NewPodcastsHandler(podcastFeedSvc, d.cache)

	audioBriefingPresetsSvc := service.NewSettingsService(userSettingsRepo, userRepo, audioBriefingRepo, summaryAudioRepo, aivisModelRepo, obsidianExportRepo, llmUsageRepo, openRouterModelOverrideRepo, d.secretCipher, d.githubApp)
	audioBriefingPresetsSvc.SetAudioBriefingPresetRepo(audioBriefingPresetRepo)
	audioBriefingPresetsH := handler.NewAudioBriefingPresetsHandler(audioBriefingPresetsSvc)

	briefingH := handler.NewBriefingHandler(itemRepo, repository.NewBriefingSnapshotRepo(db), repository.NewReadingStreakRepo(db), userSettingsRepo, llmUsageRepo, d.secretCipher, d.worker, d.cache, d.keyProvider)
	...
	return appModule{
		...
		registerAPI: func(r chi.Router) {
			...
			r.Route("/audio-briefing-presets", func(r chi.Router) {
				r.Get("/", audioBriefingPresetsH.List)
				r.Post("/", audioBriefingPresetsH.Create)
				r.Put("/{id}", audioBriefingPresetsH.Update)
				r.Delete("/{id}", audioBriefingPresetsH.Delete)
			})
...
func buildLLMModelsModule(d *appDeps) appModule {
	db := d.db
	userSettingsRepo := d.userSettingsRepo
	...
	providerModelSnapshotSyncSvc := service.NewProviderModelSnapshotSyncService(
		d.userRepo, userSettingsRepo, providerModelUpdateRepo,
		repository.NewPushNotificationLogRepo(db), d.oneSignal, d.secretCipher,
	)
	...
	return appModule{
		registerAPI: func(r chi.Router) {
			...
			r.Route("/gemini-tts-voices", func(r chi.Router) {
				r.Get("/", geminiTTSVoicesH.List)
			})
		},
	}
}

func buildAskModule(d *appDeps) appModule {
	db := d.db
	itemRepo := d.itemRepo
	userSettingsRepo := d.userSettingsRepo
	llmUsageRepo := d.llmUsageRepo
	...
	askH := handler.NewAskHandler(itemRepo, userSettingsRepo, llmUsageRepo, d.secretCipher, d.worker, d.openAI, d.cache, d.keyProvider)
...
func buildDigestModule(d *appDeps) appModule {
	db := d.db
...
func buildLLMUsageModule(d *appDeps) appModule {
	db := d.db
	llmUsageRepo := d.llmUsageRepo
	llmExecutionRepo := repository.NewLLMExecutionEventRepo(db)
	llmValueMetricsRepo := repository.NewLLMValueMetricsRepo(db)
	llmUsageH := handler.NewLLMUsageHandlerWithValueMetrics(llmUsageRepo, llmExecutionRepo, llmValueMetricsRepo, d.cache)
...
func buildDashboardModule(d *appDeps) appModule {
	db := d.db
	sourceRepo := d.sourceRepo
	itemRepo := d.itemRepo
	digestRepo := repository.NewDigestRepo(db)
	llmUsageRepo := d.llmUsageRepo
	dashboardH := handler.NewDashboardHandler(sourceRepo, itemRepo, digestRepo, llmUsageRepo, d.cache)
...
func buildReviewsModule(d *appDeps) appModule {
	db := d.db
	reviewQueueRepo := repository.NewReviewQueueRepo(db)
...
func buildInternalModule(d *appDeps) appModule {
	db := d.db
	userRepo := d.userRepo
	...
	internalH := handler.NewInternalHandler(userRepo, userIdentityRepo, obsidianExportRepo, itemInngestRepo, digestInngestRepo, userSettingsRepo, d.secretCipher, d.eventPublisher, db, d.cache, d.worker, d.oneSignal, d.githubApp, d.search)

	inngestHandler := inngestfn.NewHandler(db, d.worker, d.resend, d.oneSignal, obsidianExportSvc, d.cache, d.search, d.keyProvider)
...
func preloadDynamicModels(ctx context.Context, d *appDeps) {
	db := d.db
	openRouterModelRepo := repository.NewOpenRouterModelRepo(db)
	poeModelRepo := repository.NewPoeModelRepo(db)
	featherlessModelRepo := repository.NewFeatherlessModelRepo(db)
...
func registerHealthEndpoint(r chi.Router) {
