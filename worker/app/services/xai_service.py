...
_XAI_PRICING_SOURCE_VERSION = "xai_static_2026_03"
...
def _pricing_for_model(model: str, purpose: str) -> dict:
    ...
    source = str(base.get("pricing_source") or _XAI_PRICING_SOURCE_VERSION)
    prefix = f"XAI_{purpose.upper()}_"
    override_map = {
...
def _llm_meta(model: str, purpose: str, usage: dict) -> dict:
    ...
    return with_execution_failures({
        ...
        "pricing_source": pricing.get("pricing_source", _XAI_PRICING_SOURCE_VERSION),
...
def _chat_json(
    ...
    *,
    ...
    req_timeout = timeout_sec if timeout_sec and timeout_sec > 0 else _env_timeout_seconds("XAI_TIMEOUT_SEC", 300.0)
    attempts = max(1, int(os.getenv("XAI_RETRY_ATTEMPTS", "3") or "3"))
    base_sleep_sec = _env_timeout_seconds("XAI_RETRY_BASE_SEC", 0.5)
    return run_chat_json(
        prompt,
        model,
        api_key,
        url=os.getenv("XAI_API_BASE_URL", "https://api.x.ai/v1/chat/completions"),
...
def extract_facts(title: str | None, content: str, model: str, api_key: str) -> dict:
    ...
    if not facts:
        raise RuntimeError(f"xai extract_facts parse failed: response_snippet={text[:500]}")
...
def summarize(title: str | None, facts: list[str], source_text_chars: int | None = None, model: str = "grok-4-1-fast-non-reasoning", api_key: str = "") -> dict:
    ...
    return finalize_summary_result(
        ...
        error_prefix="xai summarize parse failed",
        response_text=text,
    )
...
def translate_title(title: str, model: str = "grok-4-fast-non-reasoning", api_key: str = "") -> dict:
    src = (title or "").strip()
    if not src:
...
def compose_digest(digest_date: str, items: list[dict], model: str, api_key: str) -> dict:
    ...
    text, usage = _chat_json(
        ...
        response_schema=task["schema"],
        schema_name="digest",
        timeout_sec=_env_timeout_seconds("XAI_COMPOSE_DIGEST_TIMEOUT_SEC", 300.0),
    )
    subject, body = parse_digest_result(text, error_prefix="xai compose_digest parse failed")
...
def ask_question(query: str, candidates: list[dict], model: str, api_key: str) -> dict:
    ...
    result = parse_ask_result(text, candidates, error_prefix="xai ask missing answer")
...
def compose_digest_cluster_draft(cluster_label: str, item_count: int, topics: list[str], source_lines: list[str], model: str, api_key: str) -> dict:
    ...
    try:
        ...
        _log.warning("xai compose_digest_cluster_draft primary attempt failed: %s", exc)
        try:
            ...
            _log.warning("xai compose_digest_cluster_draft fallback failed: %s", retry_exc)
