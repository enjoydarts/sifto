...
_OPENROUTER_PRICING_SOURCE_VERSION = "openrouter_snapshot"
...
def _pricing_for_model(model: str, purpose: str) -> dict:
    ...
    source = str(base.get("pricing_source") or _OPENROUTER_PRICING_SOURCE_VERSION)
    prefix = f"OPENROUTER_{purpose.upper()}_"
    override_map = {
...
def _chat_json(
    ...
    *,
    ...
    req_timeout = timeout_sec if timeout_sec and timeout_sec > 0 else _env_timeout_seconds("OPENROUTER_TIMEOUT_SEC", 300.0)
    ...
    text, usage = run_chat_json(
        ...
        model,
        api_key,
        url=os.getenv("OPENROUTER_API_BASE_URL", "https://openrouter.ai/api/v1/chat/completions"),
...
def extract_facts(title: str | None, content: str, model: str, api_key: str) -> dict:
    ...
    if not facts:
        raise RuntimeError(f"openrouter extract_facts parse failed: response_snippet={text[:500]}")
...
def summarize(title: str | None, facts: list[str], source_text_chars: int | None = None, model: str = "openrouter/auto", api_key: str = "") -> dict:
    ...
    return finalize_summary_result(
        ...
        error_prefix="openrouter summarize parse failed",
        response_text=text,
    )
...
def translate_title(title: str, model: str = "openrouter/auto-flash", api_key: str = "") -> dict:
    src = (title or "").strip()
    if not src:
...
def compose_digest(digest_date: str, items: list[dict], model: str, api_key: str) -> dict:
    ...
    text, usage = _chat_json(
        ...
        response_schema=task["schema"],
        schema_name="digest",
        timeout_sec=_env_timeout_seconds("OPENROUTER_COMPOSE_DIGEST_TIMEOUT_SEC", 300.0),
    )
    subject, body = parse_digest_result(text, error_prefix="openrouter compose_digest parse failed")
...
def ask_question(query: str, candidates: list[dict], model: str, api_key: str) -> dict:
    ...
    result = parse_ask_result(text, candidates, error_prefix="openrouter ask missing answer")
...
def compose_digest_cluster_draft(cluster_label: str, item_count: int, topics: list[str], source_lines: list[str], model: str, api_key: str) -> dict:
    ...
    try:
        ...
        _log.warning("openrouter compose_digest_cluster_draft primary attempt failed: %s", exc)
        try:
            ...
            _log.warning("openrouter compose_digest_cluster_draft fallback failed: %s", retry_exc)
...
def compose_ai_navigator_brief(persona: str, candidates: list[dict], intro_context: dict, model: str, api_key: str) -> dict:
    ...
    text, usage = _chat_json(task["prompt"], model, api_key, max_output_tokens=6400, response_schema=task["schema"], schema_name="ai_navigator_brief", temperature=task["sampling_profile"]["temperature"], top_p=task["sampling_profile"]["top_p"])
...
def suggest_feed_seed_sites(existing_sources: list[dict], preferred_topics: list[str], positive_examples: list[dict] | None, negative_examples: list[dict] | None, model: str, api_key: str) -> dict:
    ...
    if len(out) == 0:
        try:
            ...
            _log.warning("openrouter suggest_feed_seed_sites rescue failed: %s", exc)
