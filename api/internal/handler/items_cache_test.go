...
func TestItemListCacheKeyUsesVersion(t *testing.T) {
	...
	key, err := handler.itemsListCacheKey(context.Background(), "u1", "summarized", "src-1", "go", true, false, true, false, "score", 2, 50)
	if err != nil {
		t.Fatalf("itemsListCacheKey returned error: %v", err)
	}
	want := "v1:items:list:u1:v=7:status=summarized:source=src-1:topic=go:unread=true:read=false:fav=true:later=false:sort=score:page=2:size=50"
	if key != want {
