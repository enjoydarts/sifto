...
const secondaryLinks = [
  ...
  { href: "/deepinfra-models", labelKey: "nav.deepinfraModels", icon: Link2 },
...
function NavShell({ displayName, hasSignedInUser, onSignOut }: SharedNavProps) {
  ...
  const moreSubmenuGroups = [
    ...
    },
    {
      ...
      items: visibleSecondaryLinks.filter((link) =>
        ["/llm-usage", "/llm-analysis", "/provider-model-snapshots", "/poe-models", "/openrouter-models", "/deepinfra-models", "/featherless-models", "/aivis-models", "/fish-models", "/prompt-admin"].includes(link.href),
      ),
    },
  ];
