...
export default function LLMAnalysisPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard>
        <SectionTitle
          eyebrow={t("llmAnalysis.section.mix", "Provider Mix")}
          title={t("llmAnalysis.section.mixTitle", "種別ごとの provider 構成比")}
          description={t("llmAnalysis.section.mixDescription", "provider × model × 種別 のうち、まず provider の偏りだけを抜き出して見ます。")}
        />
        <section className="rounded-[18px] border border-[var(--color-editorial-line)] bg-[var(--color-editorial-panel-strong)] p-4">
      ...
      <SectionCard>
        <SectionTitle
          eyebrow={t("llmAnalysis.section.quality", "Reliability")}
          title={t("llmAnalysis.section.qualityTitle", "failure rate の外れ値")}
          description={t("llmAnalysis.section.qualityDescription", "高頻度なのに失敗率が高いモデルを、コストとは別軸で拾います。")}
        />
        <section className="rounded-[18px] border border-[var(--color-editorial-line)] bg-[var(--color-editorial-panel-strong)] p-4">
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="rounded-[14px] border border-[var(--color-editorial-line)] bg-white px-3 py-2 shadow-[var(--shadow-dropdown)]">
                          ...
                          <div className="mt-2 grid gap-1 text-xs text-[var(--color-editorial-ink-soft)]">
                            <div>{t("llm.totalCalls")}: {fmtNum(row.calls)}</div>
                            <div>{t("llmAnalysis.quality.failureRate")}: {row.failure_rate_pct == null ? "—" : `${row.failure_rate_pct.toFixed(1)}%`}</div>
                            <div>{t("llmAnalysis.quality.retryRate")}: {row.retry_rate_pct == null ? "—" : `${row.retry_rate_pct.toFixed(1)}%`}</div>
                          </div>
                        </div>
                      );
                    }}
