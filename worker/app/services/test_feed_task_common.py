...
class FeedTaskCommonTests(unittest.TestCase):
    ...
    def test_build_audio_briefing_script_task_uses_full_persona_and_fixed_article_order(self):
        ...
        self.assertIn("headline は 4〜5文", prompt)
        self.assertIn("summary_intro は 5〜7文", prompt)
        self.assertIn("commentary は 11〜12文", prompt)
        self.assertIn("少なくとも 4文は使い", prompt)
        self.assertIn("少なくとも 5文は使い", prompt)
        self.assertIn("少なくとも 11文は使い", prompt)
    ...
    def test_build_audio_briefing_script_task_uses_tighter_ranges_for_dense_episode(self):
        ...
        prompt = task["prompt"]
        self.assertIn("headline は 2〜3文", prompt)
        self.assertIn("summary_intro は 3〜4文", prompt)
        self.assertIn("commentary は 4〜5文", prompt)
