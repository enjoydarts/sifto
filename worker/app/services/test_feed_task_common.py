...
class FeedTaskCommonTests(unittest.TestCase):
    ...
    def test_build_audio_briefing_script_task_uses_full_persona_and_fixed_article_order(self):
        ...
        self.assertIn("共通点や流れを具体化してよい", prompt)
        ...
        self.assertIn("headline では、その記事の短い要約をしてよい。1〜3個の要点に絞って、その記事の芯が短く伝わるようにする", prompt)
        self.assertIn("headline は短い導入見出しとして使い", prompt)
        self.assertIn("commentary では記事の長い説明、論点整理、一般論への脱線は避ける", prompt)
        self.assertIn("反応の理由、軽い背景説明、比較、自分ならどう見るかまで話してよい", prompt)
        self.assertIn("headline で置いた要約をなぞり直さず、反応、理由、比較に進む", prompt)
        self.assertIn("解説調に見えやすい運びを避ける", prompt)
        self.assertIn("短い書き方の例", prompt)
        self.assertIn('"item_id": "example-item"', prompt)
        self.assertIn("headline で何の記事かを短く置き、commentary でそのペルソナの反応を書く", prompt)
