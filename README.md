...
登録した RSS フィードと単発 URL を自動収集し、本文抽出・事実抽出・要約を行い、Digest メールとブリーフィングで消化しやすく整えるパーソナル情報収集サービス。ブリーフィング、クイックトリアージ、インラインリーダー、トピックパルスなどの UX 機能でインプットの効率化を支援する。
...
| `NEXTAUTH_URL` | アプリのベース URL（Digest 内リンク生成用。名称整理候補） | — |
...
Web・API・Worker に Sentry を導入し、Redis は API の JSON キャッシュと Worker の Gemini コンテキストキャッシュに利用する。Push 通知は OneSignal を利用し、重要な記事候補を通知できる。
