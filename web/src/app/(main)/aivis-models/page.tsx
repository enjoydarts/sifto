...
export default function AivisModelsPage() {
  ...
  const [data, setData] = useState<AivisModelsResponse | null>(null);
  ...
  const filteredModels = useMemo(() => {
    ...
    return models.filter((model) =>
      [
        ...
        model.voice_timbre,
        userName(model),
        ...(model.tags_json ?? []).map((tag) => tag.name),
        ...(model.speakers_json ?? []).map((speaker) => speaker.name),
      ]
        .join(" ")
        .toLowerCase()
        .includes(q)
              ...
              <div className="overflow-x-auto">
                <table className="min-w-[920px] divide-y divide-[var(--color-editorial-line)] text-sm">
                  ...
                  <tbody className="divide-y divide-[var(--color-editorial-line)] bg-white">
                    {loading ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-sm text-[var(--color-editorial-ink-soft)]">
                          {t("common.loading")}
                        </td>
                      </tr>
                      ...
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-sm text-[var(--color-editorial-ink-soft)]">
                          {activeSection === "removed" ? t("aivisModels.noRemovedModels") : t("aivisModels.noModels")}
                        </td>
                      </tr>
                    ) : (
                      filteredModels.map((model) => (
                        <tr
                          key={`${activeSection}-${model.aivm_model_uuid}`}
                  ...
                  <section className="rounded-[22px] border border-[var(--color-editorial-line)] bg-white p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(selectedModel.tags_json ?? []).length > 0 ? (
                        selectedModel.tags_json.map((tag) => (
                          <span key={`${selectedModel.aivm_model_uuid}-${tag.name}`} className="rounded-full border border-[var(--color-editorial-line)] px-3 py-1 text-xs text-[var(--color-editorial-ink-soft)]">
                            {tag.name}
                          </span>
                        ))
                      ) : (
                        ...
                        <div key={speaker.aivm_speaker_uuid} className="rounded-[18px] border border-[var(--color-editorial-line)] bg-[var(--color-editorial-panel)] p-4">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {speaker.styles.map((style) => (
                              <span key={`${speaker.aivm_speaker_uuid}-${style.local_id}`} className="rounded-full border border-[var(--color-editorial-line)] bg-white px-3 py-1 text-xs text-[var(--color-editorial-ink-soft)]">
                                {style.name} · #{style.local_id}
                              </span>
                            ))}
                          </div>
                        </div>
